...
function drawLolliPopChart() {
  ...
  g.append('g').call(xAxis).attr('transform', `translate(0,${actualHeight})`);
  g.append('text')
    .attr('x', actualWidth / 2)
    .attr('y', actualHeight + 40)
    .text('Year');
  ...
  g.selectAll('mCircle')
    ...
    .duration(1000)
    .attr('cx', function (d) {
      return xRange(d.Year) - 4;
    })
    .attr('cy', function (d) {
      return yRange(d[selectedCountry]);
    })
    .attr('r', 4)
    .style('fill', '#122740');
  // .style('stroke','red');
  ...
  g.selectAll('mLine')
    ...
    .attr('y1', function (d) {
      return yRange(d[selectedCountry]) + 4;
    })
    .attr('y2', yRange(0))
    // .style('fill', '#122740')
    .style('stroke', '#122740');
